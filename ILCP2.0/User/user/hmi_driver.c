...
*/
void set_text_scroll(u16 screen_id, u16 control_id, u16 speed)
{
	BEGIN_CMD();
	TX_16(0xB116);
	TX_16(screen_id);
...
//批量清空参数四个页面的画面按钮状态  screen_id:当前图片ID  ctrl_id0:控件ID 
...
//弹出系统键盘  参数：关闭键盘时想要返回的界面
void eject_keyboard_set(u16 screen_id)
{
												//x坐标    //y坐标  //全键盘 //Option //录入长度
	u8 e_system_keyboard[] = {0xEE,0x86,0x01,0x00,0x00,0x00,0x00, 0x01,    0x00,   0xFF,   0xFF,0xFC,0xFF,0xFF};
	if(screen_id != 0xFFFF)
		set_screen(screen_id);

	ui_updata_tx(e_system_keyboard,sizeof(e_system_keyboard));  //弹出系统键盘
}
