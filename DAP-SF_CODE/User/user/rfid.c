	...
	switch(b[2])
	{
		...
		case 0x77:              //读取RFID的数据
		  //若有接收
			...
			if(b[8] == 0)        //若读取到数据
			{
				memcpy(&rf1_data_buf[read_offset],b+9,16);    //拷贝一个块的数据
				read_addr += 4;      //读取地址偏移
				read_offset += 16;   //存地址偏移
				connect_cnt = 5;
...
int rfid1_write_data(u8* buf, u8 addr,u8 len)
{
	...
	while(write_num)
	{
		...
		if(write_flag == 0)
		{
			...
			memcpy(&cmd_write[9],buf+write_offset,4);
