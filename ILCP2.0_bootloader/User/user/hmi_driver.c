...
/*! 
*  \brief   显示切换图标
*  \param   screen_id 画面编号  control_id 控件编号  icon_id 图标文件的某一帧id
*/
void switch_icon(u16 screen_id, u16 control_id, u8 icon_id)
{
	BEGIN_CMD();
	TX_16(0xB123);
	TX_16(screen_id);
...
//批量清空参数四个页面的画面按钮状态  screen_id:图片ID  ctrl_id0:控件ID 
void clear_button_value(u16 screen_id,u16 ctrl_id0,u16 ctrl_id1,u16 ctrl_id2)
{
	BatchBegin(screen_id);
	BatchSetButtonValue(ctrl_id0,0x00);
	BatchSetButtonValue(ctrl_id1,0x00);
	BatchSetButtonValue(ctrl_id2,0x00);
	BatchEnd();
}
//清空选中效果
void clear_button_all(u16 screen_id,u16 ctrl_id0,u16 ctrl_id1,u16 ctrl_id2,u16 ctrl_id3)
{
	BatchBegin(screen_id);
	BatchSetButtonValue(ctrl_id0,0x00);
	BatchSetButtonValue(ctrl_id1,0x00);
	BatchSetButtonValue(ctrl_id2,0x00);
	BatchSetButtonValue(ctrl_id3,0x00);
	BatchEnd();
}

//由于FATFS按gbk编码读取文本，屏幕显示某些控件需要按uncode编码属性
//只能处理字符
void my_gbktouni(char* destin,char* source,u32 len)
{
	const char symbol[] = {0xC2,0xAE,0x20};  //特殊字符 注册商标
	char *s_addr = NULL;
	s_addr = strstr(source,symbol);
	if( s_addr )
	{
		int i = 0,j = 0;
		int front_len = s_addr - source;
...
//设置unicode编码文本显示
void set_text_value(uint16 screen_id,uint16 control_id,uchar *str,u32 len)
{
	u8 destin[100] = {0};
    BEGIN_CMD();
    TX_8(0xB1);
    ...
    TX_16(screen_id);
    TX_16(control_id);
	memset(destin,0x00,sizeof(destin));
	my_gbktouni((char*)destin,(char*)str,len);
	SendNU8(destin,len*2);
    END_CMD();
}

//清除文本内容
void clear_text_value(uint16 screen_id,uint16 control_id)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x10);
...
//切换画面
void set_screen(u16 screen_id)
{
	if(ui_id.cur_picture != screen_id)
	{
		ui_id.pre_picture = ui_id.cur_picture;
		ui_id.cur_picture = screen_id;
		SetScreen(screen_id);
	}
}
