...
void save_para(void)
{
	if(strlen((const char*)user_custom[custom_page.select_num].resin) == 0)  
	{
		pop_tips(30,(u8*)WARNING,(u8*)EN_NAME);
		return;
	}
	else if(user_custom[custom_page.select_num].time[0] == 0)    
	{
...
u32 input_curing_time(void)
{
	...
	{
		if(user_custom[custom_page.select_num].up_uv[i] != 0 || user_custom[custom_page.select_num].down_uv[i] != 0)
		{
			curing_time += user_custom[custom_page.select_num].time[i];
		}
	}
	return curing_time;
}
...
void show_cur_custom_page(u16 cur_page)
{
	...
	{
//		clear_text_value(6,9 + i);
//		clear_text_value(6,20 + i);
//		switch_icon(6,25+i,0);
		if( 0 != strlen((const char*)user_custom[i + (cur_page*TOTAL_NUM_LINE_PAGE)].resin) )
		{
			...
			user_custom[i + (cur_page*TOTAL_NUM_LINE_PAGE)].time[3] + user_custom[i + (cur_page*TOTAL_NUM_LINE_PAGE)].time[4];	
...
void custom_input_pro(u16 touch_id,u8 *p,int len)
{
	...
	if( touch_id == 3 )   
	{
		memset(user_custom[custom_page.select_num].resin,0x00,sizeof(user_custom[custom_page.select_num].resin));
		memcpy(user_custom[custom_page.select_num].resin,p+8,chars_len);  
	}
	else if( touch_id >= 4 && touch_id <= 7 )  
	{
		user_custom[custom_page.select_num].up_uv[touch_id - 4] = get_str_num((char*)(p+8),chars_len);
	}
	else if( touch_id >= 8 && touch_id <= 11 ) 
	{
		user_custom[custom_page.select_num].down_uv[touch_id - 8] = get_str_num((char*)(p+8),chars_len);
	}
	else if( touch_id >= 12 && touch_id <= 15 )  
	{
		user_custom[custom_page.select_num].heat[touch_id - 12] = get_str_num((char*)(p+8),chars_len);
	}
	else if( touch_id >= 16 && touch_id <= 19 )  
	{
		user_custom[custom_page.select_num].time[touch_id - 16] = get_str_num((char*)(p+8),chars_len);
	}
}
...
DSTATUS write_user_parameter(const TCHAR* path,const void *buff,u32 len)
{
	...
	f_stat = f_open(&f_user, path, FA_CREATE_NEW | FA_WRITE);  
	if(f_stat == FR_OK)
	{
		f_stat = f_write(&f_user,buff,len,&b_write);
	}
	f_close(&f_user);
	return f_stat;
}
